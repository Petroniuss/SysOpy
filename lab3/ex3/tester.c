...
int generateTest(int min, int max, int testNum, int cpuTimeLimit, int virtualMemoryLimit) {
    ...
    if (pid == 0) {
        ...
        char* cpuTimeLimitStr = numberToString(cpuTimeLimit);
        char* virtualMemoryLimit = numberToString(cpuTimeLimit);
        ...
        execl("./parallel", "./parallel", configFilename, workersStr, MAX_EXEC_TIME, EXECUTION_FLAG, cpuTimeLimitStr, virtualMemoryLimit, NULL);
...
int main(int argc, char* argv[]) {
    ...
    int min = atoi(argv[2]);
    int max = atoi(argv[3]);
    int  cpuTimeLimit       = atoi(argv[4]);
    int  virtualMemoryLimit = atoi(argv[5]); 

    srand(time(NULL));
    int passed = 0;

    for (int i = 1; i <= testNumber; i++) {
        passed += generateTest(min, max, i, cpuTimeLimit, virtualMemoryLimit);
    }
