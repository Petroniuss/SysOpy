...
void find(FindArgs* args, char* path, int depth) {
    ...
    char* relativePath = (char*) malloc(sizeof(char) * PATH_MAX_SIZE);
    ...
    for(struct dirent* dirent = readdir(dir); dirent != NULL; dirent = readdir(dir)) {
        ...
        char* relativePath = concatWithSeparator(path, filename, "/"); 
        ...
        if (S_ISREG(statBuffer.st_mode)) {
            ...
            if (S_ISLNK(statBuffer.st_mode)) {
                ...
                // Here we're spawning new processes !!!
                // -------------------------------------
                int forkedPid = fork();
                if (forkedPid == 0) {
                    // Child process executes ls -l from current directory
                    int childPid = getpid(); 
                    printf("Pid: %d, Relative path: %s\n", childPid, relativePath);
                    execl("/bin/ls", "ls", relativePath, "-l", NULL);
                } else {
                    // Parent process waits, so that ls won't interleave with find.
                    waitpid(forkedPid, NULL, 0);
                    find(args, relativePath, depth + 1);
                }
            }
